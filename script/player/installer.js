import { DeliveryMethod } from "../_shared/common.js";
...
let _inputPackage;
let _inputLibrary;
async function fetchBLOBAsync(url, apiKey, progress) {
...
async function onInputPackageChange() {
    if (_inputPackage.files.length >= 1) {
        ...
        let bPermission = await _refBookMan.invokeMethodAsync("requestInstallProceed", fileinfo);
        if (bPermission) {
            const progress = new ProgressControl(_refBookMan, "refreshInstallProgress");
            ...
            try {
                ...
                if (existing) {
                    ...
                    let bPermission = await _refBookMan.invokeMethodAsync("requestInstallComplete", candidateRaw.getCase());
                    if (bPermission) {
                        installed = await storePackage(candidateRaw, progress);
                    }
                }
...
export async function installFromService(uri, book, serviceUrl, apiKey) {
    let progress = new ProgressControl(_refBookMan, "refreshInstallProgress");
    let blobPackage;
    let fileInfo;
    try {
        [fileInfo, blobPackage] = await fetchBLOBAsync(uri, apiKey, progress);
    ...
    if (!bPermission)
        return;
    progress = new ProgressControl(_refBookMan, "refreshInstallProgress");
    progress.setSegment(70);
    let internals = undefined;
    try {
...
export async function updateFromService(uri, book, serviceUrl, apiKey) {
    let progress = new ProgressControl(_refBookMan, "refreshUpdateProgress");
    let blobPackage;
    let fileInfo;
    try {
        [fileInfo, blobPackage] = await fetchBLOBAsync(uri, apiKey, progress);
        ...
        _refBookMan.invokeMethodAsync("informUpdateError", Helper.extractMessage(err));
        return;
    }
    let bPermission = await _refBookMan.invokeMethodAsync("requestUpdateProceed", fileInfo);
    if (!bPermission)
        return;
    progress = new ProgressControl(_refBookMan, "refreshUpdateProgress");
    progress.setSegment(70);
    let internals = undefined;
    try {
        const file = new File([blobPackage], fileInfo.name, {
        ...
        const packOld = await appdb.get("packs", candidateRaw.stored.id);
        candidateRaw.stored.installed = packOld.installed;
        candidateRaw.stored.updated = Date.now();
        let bPermission = await _refBookMan.invokeMethodAsync("requestUpdateComplete", candidateRaw.getCase());
        if (bPermission) {
            const installed = await storePackage(candidateRaw, progress);
            setTimeout(() => {
                progress.done();
                _refBookMan.invokeMethodAsync("informUpdateFinish", installed, serviceUrl, candidateRaw.stored.updated);
            }, 200);
        }
    }
    catch (err) {
